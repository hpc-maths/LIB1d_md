...
class Orchestrator:
  ...
  def __init__(self, coupler, NMAX):
    ...
    # create logger
    self.logger = logging.getLogger("md_sim.coupling")
    ...
    self.md_sim_ordering = list(range(coupler.nSubsystems)) # solver ordering for WR iteration scheme (only meaningful if Gauss-Seidel mode is activated)
  ...
  def _step_forward(self, t, yn, dt, values_last_iterate, last_outs=None, rtol=None):
    ...
    if self.subsystemsIntegratedInSingleCall: # all subsystems are integrated in a Jacobi manner at once
        ...
        outs = [None for i in range(len(self.md_sim_ordering))]
        newy = [None for i in range(len(self.md_sim_ordering))]
        for it,isolv in enumerate(self.md_sim_ordering):
  ...
  def perform_step(self, y0, t, dt, atol_iter=None, rtol_iter=None, bDebug=False,
        ...
        elif self.interfaceSolver=="explicit":  # no iteration --> explicit md_sim
            # TODO: improve
...
def process_step_info(outmd_sim):
