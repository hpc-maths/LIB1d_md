...
from scipy.sparse import csc_matrix, issparse, eye, csr_matrix
...
BPRINT = False
spFormatMatrix = 'csr'
sparse_matrix_constructor = csr_matrix
...
class RadauDAE(OdeSolver):
    ...
    def __init__(self, fun, t0, y0, t_bound, max_step=np.inf,
        ...
        if issparse(self.J):
            ...
            I = eye(self.n, format=spFormatMatrix)
        else:
            def lu(A):
                self.nlu += 1
        ...
        if not (mass is None):
            if issparse(mass):
                self.mass_matrix = sparse_matrix_constructor(mass)
        ...
        if jac is None:
            if sparsity is not None:
                if issparse(sparsity):
                    sparsity = sparse_matrix_constructor(sparsity)
            ...
            self.njev = 1
            if issparse(J):
                J = sparse_matrix_constructor(J)

                def jac_wrapped(t, y, _=None):
                    self.njev += 1
                    return sparse_matrix_constructor(jac(t, y), dtype=float)
            ...
            if issparse(jac):
                J = sparse_matrix_constructor(jac)
            else:
