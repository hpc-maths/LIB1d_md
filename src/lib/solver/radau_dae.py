...
class progress_wrapper():
    def __init__(self, npoints=1000):
        ...
        self.pbar = range(npoints) #tqdm(range(npoints), bar_format='{l_bar}{bar}| {elapsed}>{remaining}{postfix}')
        # self.nupdate = 0
        # self.update(0)
...
class RadauDAE(OdeSolver):
    ...
    def __init__(self, fun, t0, y0, t_bound, max_step=np.inf,
        ...
        self.mass = self._validate_mass_matrix(mass)
            ...
            while not converged:
                ...
                if LU_real is None or LU_complex is None:
                  ...
                  try:
                      ...
                      LU_real    = self.lu( self.hscale @ (MU_REAL    / h * self.mass - J) )
                      LU_complex = self.lu( self.hscale @ (MU_COMPLEX / h * self.mass - J) )
	    ...
	    ## End of the convergence loop
            ...
            if self.constant_dt:
                ...
                if (self.bAlwaysApply2ndEstimate and error_norm > 1) or (rejected and error_norm > 1): # try with stabilised error estimate
                    if BPRINT:
                      print('\t rejected')
                    error = self.solve_lu(LU_real, self.fun(t, y + error) + self.mass.dot(ZE)) # error is not corrected for algebraic variables
    ...
    def solve_collocation_system(self, t, y, h, Z0, norm_scale, tol,
        ...
        for k in range(self.NEWTON_MAXITER):
            ...
            # compute residuals
            f_real    = F.T.dot(TI_REAL)    - M_real    * self.mass.dot( W[0] )
            f_complex = F.T.dot(TI_COMPLEX) - M_complex * self.mass.dot( W[1] + 1j * W[2] )
            if BPRINT:
