...
from src.lib.rhapsopy import coupling 
...
class BaseCoupler(coupling.BaseCoupler):
    ...
    def __init__(self, options1, options2, coupling_modes):
        ...
        if self.options1['sim_type']=="md_sim_flux" and self.options2['sim_type']=="md_sim_flux":
           ...
           self.nCouplingVars = 4 # number of coupling variables
            ...
            print(self.options1['sim_type'], self.options2['sim_type'])
        ...
        self.manual_subsolve_tol = False
    ...
    def _feedOptions(self): 
        ...
        if self.options1['sim_type']=="md_sim_flux" and self.options2['sim_type']=="md_sim_flux":
           ...
           self.options1['coupling_vars'] = lambda t : np.r_[self.predictors['ce'].evaluate(t), self.predictors['phie'].evaluate(t)]
           self.options2['coupling_vars'] = lambda t : np.r_[self.predictors['cs'].evaluate(t), self.predictors['phis'].evaluate(t)]
        else:
            raise Exception("No coupling condition specified")
    ...
    def integrateSingleSubsystem(self, isolv, t0, y0, dt, preds, rtol=None, bDebug=False):
        ...
        if rtol is None or self.manual_subsolve_tol:
        ...
        if self.adaptive_subsolves:    
            ...
            if not current_out.success:
                ...
                e = current_out.message
                current_out.message = f"Adaptive subsolve in {'e' if isolv==0 else 's'} failed with rtol {rtol} due to: " + e
                
                raise coupling.ExceptionWhichMayDisappearWhenLoweringDeltaT(f"Adaptive subsolve in {'e' if isolv==0 else 's'} failed with rtol {rtol} due to: " + current_out.message)
        else:
