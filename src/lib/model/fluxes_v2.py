...
# -*- coding: utf-8 -*-
"""
Created on Tue Feb 06 11:40:04 2024

@author: ali.asad

A change in flux definition where gradients at the interface is defined using
interface variables instead of i_BV
"""
import numpy as np
...
def flx_ie_f(t, c, phi, v_aux_a, v_aux_c, options_electrolyte, options_cathode, alg=None):
    ...
    flx_ie[0] = ( kappa_e * (phi[0] - v_aux_a[1]) / (dx[0]/2.)
                 - (Lambda/phi_c) * kappa_e  
                 * (c[0] + v_aux_a[0]) / (2. * c[0] * v_aux_a[0])
                 * (c[0] - v_aux_a[0]) / (dx[0]/2.) )
    flx_ie[1:-1] = (kappa_e*(phi[1:]-phi[:-1])/dx[:] - (Lambda/phi_c)*kappa_e*c_inv_mean[:]*(c[1:]-c[:-1])/dx[:])
    flx_ie[-1] = ( kappa_e * (v_aux_c[1] - phi[-1]) / (dx[-1]/2.)
                 - (Lambda/phi_c) * kappa_e 
                 * (c[-1] + v_aux_c[0]) / (2. * c[-1] * v_aux_c[0])
                 * (v_aux_c[0] - c[-1]) / (dx[-1]/2.) )
...
def flx_Ne_f(t, c, phi, v_aux_a, v_aux_c, options_electrolyte, options_cathode):
    ...
    flx_Ne[0] =  (D_e*(c[0] - v_aux_a[0]) / (dx[0]/2.)
                  + (t_0_plus*kappa_e_c*phi_c/(F*D_e_c*c_e_c))*flx_ie_call[0])
    flx_Ne[1:-1] = (D_e*(c[1:]-c[:-1])/dx[:] + (t_0_plus*kappa_e_c*phi_c/(F*D_e_c*c_e_c))*flx_ie_call[1:-1])   
    flx_Ne[-1] =  (D_e*(v_aux_c[0] - c[-1]) / (dx[-1]/2.)
                  + (t_0_plus*kappa_e_c*phi_c/(F*D_e_c*c_e_c))*flx_ie_call[-1])
...
def flx_Ns_f(t, c, v_aux_c, options_electrolyte, options_cathode):
    ...
    flx_Ns[0] =  D_am*(c[0] - v_aux_c[2]) / (dx[0]/2.)
...
def flx_is_f(t, phi, v_aux_c, options_electrolyte, options_cathode, alg=None):
    ...
    flx_is[0] =  sigma_am*(phi[0] - v_aux_c[3]) / (dx_am[0]/2.)
